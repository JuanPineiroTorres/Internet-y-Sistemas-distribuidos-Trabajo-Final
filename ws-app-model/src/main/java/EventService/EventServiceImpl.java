...
public class EventServiceImpl implements EventService {
    ...
    public List<Event> findEventByFutureDateWithAttend(LocalDate futureDateIn, LocalDate futureDate, int attendance)  throws  InputValidationException{
        try (Connection connection = dataSource.getConnection()){
            return eventDao.findEventByFutureDateWithAttend(connection,futureDateIn,futureDate,attendance);
        } catch (SQLException e){
            throw  new RuntimeException(e);
        }
    }

    public void  removeEventCanceledOrAlreadyDone(long eventId) throws InstanceNotFoundException, NotCanceledOrAlreadyDoneEvent{
        try (Connection connection = dataSource.getConnection()){
            Event event=findEvent(eventId);
            LocalDateTime now=LocalDateTime.now();
            if(event.getEventState()==false || event.getCelebrationDate().isBefore(now)){
                eventDao.removeCanceledOrAlreadyDone(connection,eventId);
            } else throw new NotCanceledOrAlreadyDoneEvent(eventId);
