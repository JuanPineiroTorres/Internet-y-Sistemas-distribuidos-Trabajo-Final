...
public abstract class AbstractSqlEventDao implements SqlEventDao {
    ...
    @Override
    public List<Event> findEventByFutureDateWithAttend(Connection connection, LocalDate start, LocalDate finish, int attend) throws InputValidationException {

        String queryString = "SELECT eventId, event_name, description, celebrationDate, creationDate, "
                + "runtime, event_state, attendance, not_attendance "
                + "FROM Events "
                + "WHERE celebrationDate >= CURRENT_TIMESTAMP()  "
                + "AND attendance >= ?";
        if(attend<0) throw new InputValidationException("No pueden ser nº de respuestas negativos");
        if(finish!=null && finish.isBefore(LocalDate.now())) throw new InputValidationException("La fecha debe de ser posterior no anterior");
        if (start != null && finish != null) {
            queryString += " AND celebrationDate >= ? AND celebrationDate <= ?";
        }
        try (PreparedStatement preparedStatement = connection.prepareStatement(queryString)) {
            int i=1;
            preparedStatement.setInt(i++,attend);
            if (start != null && finish != null) {
            ...
            List<Event> eventByFutureDateWithAttend = new ArrayList<Event>();
            ...
            while (resultSet.next()) {
                ...
                Event event = new Event(eventId, event_name, description, celebrationDate,
                        creationDate, runtime, event_state, attendance, not_attendance);
                eventByFutureDateWithAttend.add(event);

            }
            return eventByFutureDateWithAttend;
        }  catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    @Override
    public void removeCanceledOrAlreadyDone(Connection connection, long eventId) throws InstanceNotFoundException{
        String queryString = "DELETE FROM Events WHERE eventId = ? AND (celebrationDate < CURRENT_TIMESTAMP() OR event_state = false)";
        try (PreparedStatement preparedStatement = connection.prepareStatement(queryString)) {
            int i = 1;
            preparedStatement.setLong(i++, eventId);
