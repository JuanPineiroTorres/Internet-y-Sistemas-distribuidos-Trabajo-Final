...
public class AppServiceTest {
    ...
    @Test
    public void  TestFindByFutureCelebrationDateandAttend() throws InputValidationException, AlreadyResponseException, InstanceNotFoundException, OutOfTimeException, AlreadyCanceledException{
        Event event = getValidEvent();
        Event event2=getValidEvent();
        Event event3=getValidEvent();
        event2.setCelebrationDate(formatearfecha("22/12/2023 22:44:45"));
        event.setEventState(false);
        event.setCelebrationDate(formatearfecha("22/12/2024 22:44:45"));
        event3.setCelebrationDate(formatearfecha("22/12/2024 22:44:45"));
        event=createEvent(event);
        event2=createEvent(event2);
        event3=createEvent(event3);
        ...
        try{
            Response response1 = getValidResponse(event.getEventId(), true);
            Response response2 = getValidResponse(event.getEventId(), true);
            Response response3 = getValidResponse(event.getEventId(), false);
            Response response4 = getValidResponse(event2.getEventId(),true);
            response4=createResponse(response4);
            response1.setEmail("JuanJUJU@gmail.com");
            response1=createResponse(response1);
            response2.setEmail("gracielaJUJU@gmail.com");
            response2=createResponse(response2);
            response3.setEmail("lucas@gmail.com");
            response3=createResponse(response3);
            responses.add(response1);
            responses.add(response2);
            responses.add(response3);
            responses.add(response4);
            events=eventService.findEventByFutureDateWithAttend(null,null,1);
            assertEquals(events.get(0).getAttendance(),2);
            assertEquals(events.size(),2);
            events=eventService.findEventByFutureDateWithAttend(formatearfecha("25/12/2023 22:44:45").toLocalDate(),formatearfecha("25/12/2025 22:44:45").toLocalDate(),1);
            assertEquals(events.size(),1);
            for (Response respons : responses) {
                removeResponse(respons.getResponseId());
            }
        } finally {
            removeEvent(event.getEventId());
            removeEvent(event2.getEventId());
    ...
    @Test
    public void DeleteEventsCanceledOrAlreadyDone() throws InstanceNotFoundException, OutOfTimeException, InputValidationException, AlreadyCanceledException, NotCanceledOrAlreadyDoneEvent {
        Event eventNoCanceledNoArleadyDone= getValidEvent();
        Event eventCanceled=getValidEvent();
        Event eventAleradyDone=getValidEvent();
        eventAleradyDone=createEvent(eventAleradyDone);
        eventAleradyDone.setCelebrationDate(formatearfecha("25/12/2022 22:44:45"));
        updateEvent(eventAleradyDone);
        eventCanceled=createEvent(eventCanceled);
        eventService.CancelEvent(eventCanceled.getEventId());
        eventCanceled=eventService.findEvent(eventCanceled.getEventId());
        eventNoCanceledNoArleadyDone=createEvent(eventNoCanceledNoArleadyDone);
        try {
            assertEquals(eventService.findEvent(eventCanceled.getEventId()), eventCanceled);
            eventService.removeEventCanceledOrAlreadyDone(eventCanceled.getEventId());
            final long eventid = eventCanceled.getEventId();
            assertThrows(InstanceNotFoundException.class, () ->
                    eventService.findEvent((eventid)));
            assertEquals(eventService.findEvent(eventAleradyDone.getEventId()), eventAleradyDone);
            eventService.removeEventCanceledOrAlreadyDone(eventAleradyDone.getEventId());
            final long eventid2 = eventAleradyDone.getEventId();
            assertThrows(InstanceNotFoundException.class, () ->
                    eventService.findEvent((eventid2)));
            final long eventid3=eventNoCanceledNoArleadyDone.getEventId();
            assertThrows(NotCanceledOrAlreadyDoneEvent.class, () ->
                    eventService.removeEventCanceledOrAlreadyDone((eventid3)));
        } finally {
            removeEvent(eventNoCanceledNoArleadyDone.getEventId());
        }
    }
